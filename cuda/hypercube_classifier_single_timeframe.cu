...
constexpr int NUM_PATCHES     = 256;     // Spatial tokens per timeframe
constexpr int NUM_TIMEFRAMES  = 4;       // Number of temporal snapshots (T)
constexpr int TOTAL_TOKENS    = NUM_PATCHES * NUM_TIMEFRAMES; // Total spatio-temporal tokens
    ...
    uint32_t seed) 
{
    ...
    bool current_state = sequence_hypercube[tid]; // Frame 0 baseline state
...
// ============================================================================
// 2. SPATIO-TEMPORAL FORWARD EXTRACTION KERNEL
// ============================================================================
...
{
    int t_idx     = blockIdx.y; // Timeframe index
    int patch_idx = blockIdx.x; // Spatial patch index
    ...
    int total_tokens, int embed_dim, int num_classes)
{
    ...
    if (d < embed_dim) {
        ...
        // Un-pool gradient across all spatio-temporal tokens
...
int main() {
    ...
    CUDA_CHECK(cudaMemset(d_sequence_hypercube, 1, seq_bytes));
    ...
    dim3 fwd_grid(NUM_PATCHES, NUM_TIMEFRAMES);
    ...
    for (int step = 1; step <= 10; ++step) {
        ...
        // Step 7: AdamW Optimizer Updates
